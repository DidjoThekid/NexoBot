...
class Moderation(commands.Cog):
    ...
    # ------------------------------------------------------------------
    # Warn system (stocké en SQLite, par serveur)
    # ------------------------------------------------------------------
    @commands.command()
    @commands.has_permissions(moderate_members=True)
    async def warn(self, ctx: commands.Context, member: discord.Member, *, reason: str = "Aucune raison fournie"):
        count = await database.add_warning(
            ctx.guild.id, member.id, str(ctx.author), reason, datetime.datetime.utcnow().isoformat()
        )
    ...
    async def warnings(self, ctx: commands.Context, member: discord.Member = None):
        ...
        entries = await database.get_warnings(ctx.guild.id, member.id)
